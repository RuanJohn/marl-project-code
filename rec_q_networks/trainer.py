    ...
    def _learn(self, 
        ...
        with tf.GradientTape() as tape:
            ...
            for agent in range(self.num_agents):
                ...
                # print(self.q_network.initial_state(self.batch_size))
                
                
                q_values, _ = snt.static_unroll(self.q_network, observations[agent],
                                                    self.q_network.initial_state(self.batch_size))
                
                target_q_values, _ = snt.static_unroll(self.target_q_network, observations[agent],
                ...
                # doing q-learning
                max_next_q_value = tf.reduce_max(target_q_values, axis = -1)
                q_out.append(selected_q_value)
                q_next_out.append(max_next_q_value)
                #reward_out.append(rewards[agent])
            ...
            #reward_out = tf.stack(reward_out)
            
            #bellman target 
            
            #comments for VDN
            #target = reward_out + self.discount * (1 - dones)
            target = rewards[:,0:-1] + self.discount * (1-dones[:,0:-1]) * q_next_out[:,1:]
            
            td_error = target - q_out[:,0:-1]
